...
internet_plug_in_dir="/Library/Internet Plug-Ins"
internet_plug_in="$internet_plug_in_dir/JavaAppletPlugin.plugin"
internet_plug_in_home="$internet_plug_in/Contents/Home"
hidden_internet_plugin_dir="$internet_plug_in_dir/hidden"
deployment_properties="deployment.properties"
while [[ ! ("$jhome" =~ corretto) ]]; do
    if [[ "$jhome" == "$internet_plug_in_home" ]]; then
        # hide the plug-in to disable it
        print "Disabling JavaAppletPlugin..."
        mkdir -p "$hidden_internet_plugin_dir"
        mv "$internet_plug_in" "$hidden_internet_plugin_dir/"
        
        # update jhome for the next go-round
        jhome=`/usr/libexec/java_home`
    else
        ...
        corretto_export="export JAVA_HOME=\"$corretto_home\""
        ...
        # we've forced it to be what we want, so we can break out of the loop now.
        break
    fi
done
